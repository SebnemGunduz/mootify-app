...
class TimerActivity : AppCompatActivity() {
    ...
    private var remainingMinuteTime = 0
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        remainingMinuteTime = 0

        intent.extras?.let {
            currentUsername = it.getString("USER", "")

            val foundUser = SQLite.select()
                .from(User::class.java)
                .where(User_Table.username.eq(currentUsername))
                .querySingle()
    ...
    private fun startCountDownTimer(millisInFuture: Long) {
        remainingMinuteTime = millisInFuture.toInt() / 1000 / 60
        countDownTimer = object : CountDownTimer(millisInFuture, 1000) {
            ...
            override fun onFinish() {
                resetTimer()
                currentUser?.dailyRemainingMinuteTime = currentUser?.dailyRemainingMinuteTime!! - remainingMinuteTime
                currentUser?.update()

                Log.i("TIMER_USER", currentUser.toString())
            }
        }
